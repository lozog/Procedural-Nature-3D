...
BranchNode::BranchNode( float bottomRadius,
				float topRadius,
				float length,
				glm::vec3 origin,
...
{
	BranchNode::heading = glm::normalize(heading);
	BranchNode::left = glm::normalize(left);
...
void BranchNode::init( ShaderProgram& m_shader, GLuint m_texture ) {
	...
	BranchNode::m_texture = m_texture;
